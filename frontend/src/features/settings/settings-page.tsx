...
import { Button, Dialog, EmptyState, Field, Input, Select, Textarea } from "../../components/ui";
...
export function SettingsPage() {
  ...
  const toggleNotifications = async (checked: boolean) => {
    ...
    if (permission === "denied") pushToast({ title: "دسترسی نوتیفیکیشن مرورگر مسدود است. یادآوری ها فقط داخل برنامه نمایش داده می شوند." });
  };
  ...
  const exportExcel = () => { if (!workspace.data) return; downloadWorkspaceExcel(workspace.data); pushToast({ title: "فایل اکسل آماده شد." }); };
  ...
  return <div className="page settings-page"><PageHeader title="تنظیمات" description="ظاهر، رفتار تقویم و حفاظت از داده های محلی را مدیریت کنید." />
    <div className="settings-shell">
      ...
      <div className="settings-content">
        ...
        {tab === "notifications" && <section className="surface settings-section"><header><BellRing size={20} /><div><h2>یادآوری و اعلان</h2><p>{isNotificationSupported() ? "اعلان ها برای هر دستگاه جداگانه فعال می شوند و یادآورها از مسیر زمان بندی سرور پردازش می شوند." : "مرورگر شما از نوتیفیکیشن پشتیبانی نمی کند؛ یادآوری ها فقط داخل برنامه نمایش داده می شوند."}</p></div></header><div className="settings-form"><label className="switch-row"><span><strong>فعال سازی اعلان های این دستگاه</strong><small>{settings.notificationsEnabled && notificationPermission === "denied" && "دسترسی مرورگر مسدود است"}{settings.notificationsEnabled && notificationPermission === "granted" && "نوتیفیکیشن مرورگر فعال است"}{!settings.notificationsEnabled && "ابتدا توضیح فارسی نمایش داده می شود و سپس اجازه مرورگر درخواست می شود."}</small></span><input type="checkbox" checked={settings.notificationsEnabled} onChange={(event) => void toggleNotifications(event.target.checked)} /></label><Field label="یادآور پیش فرض"><Select value={String(settings.notificationLeadMinutes)} disabled={!settings.notificationsEnabled} onChange={(event) => update({ notificationLeadMinutes: Number(event.target.value), notificationPreferences: { ...(settings.notificationPreferences ?? DEFAULT_SETTINGS.notificationPreferences!), defaultReminderMinutes: [Number(event.target.value)] } })}><option value="0">در موعد</option><option value="10">۱۰ دقیقه قبل</option><option value="30">۳۰ دقیقه قبل</option><option value="60">۱ ساعت قبل</option><option value="180">۳ ساعت قبل</option><option value="1440">۱ روز قبل</option></Select></Field><Field label="شروع سکوت" optional><Input type="time" value={settings.notificationPreferences?.quietHoursStart ?? ""} onChange={(event) => update({ notificationPreferences: { ...(settings.notificationPreferences ?? DEFAULT_SETTINGS.notificationPreferences!), quietHoursStart: event.target.value || null } })} /></Field><Field label="پایان سکوت" optional><Input type="time" value={settings.notificationPreferences?.quietHoursEnd ?? ""} onChange={(event) => update({ notificationPreferences: { ...(settings.notificationPreferences ?? DEFAULT_SETTINGS.notificationPreferences!), quietHoursEnd: event.target.value || null } })} /></Field><Field label="حریم خصوصی اعلان"><Select value={settings.notificationPreferences?.privacyMode ?? "full"} onChange={(event) => update({ notificationPreferences: { ...(settings.notificationPreferences ?? DEFAULT_SETTINGS.notificationPreferences!), privacyMode: event.target.value as "full" | "generic" } })}><option value="full">نمایش محتوای کامل</option><option value="generic">نمایش متن عمومی</option></Select></Field><label className="switch-row"><span><strong>اعلان چت</strong><small>پیام های مستقیم و گروهی در مرکز اعلان دیده می شوند.</small></span><input type="checkbox" checked={settings.notificationPreferences?.chatNotifications ?? true} onChange={(event) => update({ notificationPreferences: { ...(settings.notificationPreferences ?? DEFAULT_SETTINGS.notificationPreferences!), chatNotifications: event.target.checked } })} /></label><label className="switch-row"><span><strong>اعلان وظیفه و overdue</strong><small>موعدهای نزدیک و وظایف عقب افتاده را یادآوری کن.</small></span><input type="checkbox" checked={settings.notificationPreferences?.overdueNotifications ?? true} onChange={(event) => update({ notificationPreferences: { ...(settings.notificationPreferences ?? DEFAULT_SETTINGS.notificationPreferences!), overdueNotifications: event.target.checked, taskNotifications: event.target.checked } })} /></label></div></section>}

        {tab === "backup" && <section className="surface settings-section"><header><DatabaseBackup size={20} /><div><h2>پشتیبان و داده</h2><p>داده ها فقط روی دستگاه شما می مانند. پشتیبان منظم را فراموش نکنید.</p></div></header><div className="settings-actions"><Button variant="secondary" onClick={() => void backup()}><DatabaseBackup size={17} />ساخت پشتیبان</Button><Button variant="secondary" onClick={exportExcel}><Download size={17} />خروجی اکسل</Button><Button variant="secondary" onClick={() => void exportData()}><Download size={17} />خروجی JSON</Button><Button variant="secondary" onClick={() => input.current?.click()}><Upload size={17} />ورود فایل</Button><input ref={input} type="file" accept="application/json,.json" hidden onChange={(event) => void importData(event.target.files?.[0])} /></div></section>}
...
function UserManagementPanel() {
  ...
  const remove = async (user: SafeUser) => { if (user.id === currentUser?.id || !confirm(`کاربر ${user.username} و اطلاعات وابسته او حذف شود؟`)) return; await authService.softDeleteUser(user.id); await contentRepository.purgeUserData(user.id); await queryClient.invalidateQueries({ queryKey: ["workspace"] }); load(); pushToast({ title: "کاربر و اطلاعات وابسته حذف شد." }); };
...
function EditUserDialog({ user, onClose, onSaved }: { user: SafeUser | null; onClose: () => void; onSaved: () => void }) {
  ...
  const queryClient = useQueryClient();
  const [form, setForm] = useState({ firstName: "", lastName: "", phone: "", jobRole: "" as MarketingRole | "", department: "", team: "", role: "USER" as RoleKey, extraPermissions: [] as SafeUser["extraPermissions"], dataScope: "OWN" as DataScope, status: "ACTIVE" as AccountStatus, mustChangePassword: false, adminNotes: "" });
  useEffect(() => { if (!user) return; const profile = workspace.data?.userProfiles.find((item) => item.userId === user.id); setForm({ firstName: user.firstName, lastName: user.lastName, phone: user.phone ?? "", jobRole: profile?.jobRole ?? "", department: user.department ?? "", team: user.team ?? "", role: user.role, extraPermissions: user.extraPermissions, dataScope: user.dataScope, status: user.status, mustChangePassword: user.mustChangePassword, adminNotes: user.adminNotes ?? "" }); }, [user, workspace.data]);
  const update = <K extends keyof typeof form>(key: K, value: (typeof form)[K]) => setForm((current) => ({ ...current, [key]: value }));
  const togglePermission = (permission: SafeUser["extraPermissions"][number]) => update("extraPermissions", form.extraPermissions.includes(permission) ? form.extraPermissions.filter((item) => item !== permission) : [...form.extraPermissions, permission]);
  const save = async () => {
    if (!user) return;
  ...
  return <Dialog open={Boolean(user)} title="ویرایش کاربر" description="نقش، Scope داده، وضعیت و قابلیت های اضافه کاربر را تغییر دهید." onClose={onClose} wide><div className="form-grid"><Field label="نام"><Input value={form.firstName} onChange={(event) => update("firstName", event.target.value)} /></Field><Field label="نام خانوادگی"><Input value={form.lastName} onChange={(event) => update("lastName", event.target.value)} /></Field><Field label="شماره تماس" optional><Input value={form.phone} onChange={(event) => update("phone", event.target.value)} /></Field><Field label="نقش شغلی" optional><Select value={form.jobRole} onChange={(event) => update("jobRole", event.target.value as MarketingRole | "")}><option value="">بدون نقش شغلی</option>{MARKETING_ROLES.map((role) => <option key={role} value={role}>{MARKETING_ROLE_LABELS[role]}</option>)}</Select></Field><Field label="تیم" optional><Input value={form.team} onChange={(event) => update("team", event.target.value)} /></Field><Field label="واحد" optional><Input value={form.department} onChange={(event) => update("department", event.target.value)} /></Field><Field label="نقش"><Select value={form.role} onChange={(event) => update("role", event.target.value as RoleKey)}>{authService.roles().map((role) => <option value={role.key} key={role.key}>{role.label}</option>)}</Select></Field><Field label="Scope داده"><Select value={form.dataScope} onChange={(event) => update("dataScope", event.target.value as DataScope)}>{authService.dataScopes().map((scope) => <option value={scope} key={scope}>{scope}</option>)}</Select></Field><Field label="وضعیت"><Select value={form.status} onChange={(event) => update("status", event.target.value as AccountStatus)}>{(["PENDING", "ACTIVE", "INACTIVE", "LOCKED", "SUSPENDED"] as AccountStatus[]).map((status) => <option value={status} key={status}>{status}</option>)}</Select></Field><label className="switch-row"><span><strong>تغییر رمز اجباری</strong><small>در ورود بعدی کاربر باید رمز را عوض کند.</small></span><input type="checkbox" checked={form.mustChangePassword} onChange={(event) => update("mustChangePassword", event.target.checked)} /></label><Field label="یادداشت مدیریتی" optional><Textarea rows={3} value={form.adminNotes} onChange={(event) => update("adminNotes", event.target.value)} /></Field><div className="permission-grid">{ALL_PERMISSIONS.map((permission) => <label key={permission}><input type="checkbox" checked={form.extraPermissions.includes(permission)} onChange={() => togglePermission(permission)} />{permission}</label>)}</div></div><footer className="dialog-footer"><Button variant="secondary" onClick={onClose}>انصراف</Button><Button onClick={() => void save()}>ذخیره کاربر</Button></footer></Dialog>;
}
...
function CreateUserDialog({ open, onClose, onSaved }: { open: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  const [jobRole, setJobRole] = useState<MarketingRole | "">("");
