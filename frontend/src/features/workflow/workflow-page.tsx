...
const boardStatuses: ContentStatusKey[] = ["draft", "in_progress", "approved", "scheduled", "published"];
...
export function WorkflowPage() {
  ...
  const move = async ({ active, over }: DragEndEvent) => { if (!over || typeof active.id !== "string" || typeof over.id !== "string" || !over.id.startsWith("status:")) return; const content = filtered.find((item) => item.id === active.id); const next = over.id.slice(7) as ContentStatusKey; if (!content || content.status === next) return; const previous = content.status; try { await contentRepository.moveContent(content.id, content.publicationDate, next); await queryClient.invalidateQueries({ queryKey: ["contents"] }); logActivity("content.status_change", "content", content.id, content.title); pushToast({ title: `وضعیت «${content.title}» به روز شد.`, action: { label: "بازگردانی", onClick: () => { void contentRepository.moveContent(content.id, content.publicationDate, previous).then(() => queryClient.invalidateQueries({ queryKey: ["contents"] })); } } }); } catch { pushToast({ title: "تغییر وضعیت ممکن نشد." }); } };
...
function WorkflowCard({ content, onSelect }: { content: Content; onSelect: (content: Content) => void }) { const { attributes, listeners, setNodeRef, transform, isDragging } = useDraggable({ id: content.id }); const isAd = content.contentKind === "advertisement"; return <button ref={setNodeRef} type="button" className={`kanban-card ${isAd ? "kanban-card-ad" : ""}`} onClick={() => onSelect(content)} style={{ opacity: isDragging ? 0.45 : 1, transform: transform ? `translate3d(${transform.x}px, ${transform.y}px, 0)` : undefined }} {...listeners} {...attributes}><strong>{isAd && <Megaphone size={13} />} {content.title}</strong><span>{formatJalaliDate(content.publicationDate)}{content.publicationTime && ` · ${content.publicationTime}`}</span><small>{({ low: "کم", normal: "عادی", high: "زیاد", urgent: "فوری" })[content.priority]}</small><LastEditedTag updatedByName={content.updatedByName} updatedByRole={content.updatedByRole} updatedAt={content.updatedAt} /></button>; }
