import { CalendarRange, ChevronLeft, ChevronRight, Filter, Megaphone, Search } from "lucide-react";
...
type GanttRow =
  ...
  | { kind: "campaign-bar"; key: string; campaign: Campaign }
...
export function GanttPage() {
  ...
  for (const campaign of workspace.data.campaigns.filter((item) => !item.archivedAt)) {
    ...
    if (campaignSpans) rows.push({ kind: "campaign-bar", key: `bar-${campaign.id}`, campaign });
  ...
  return <div className="page gantt-page">
    ...
    {rows.length ? <section className="surface gantt-chart">
      <div className="gantt-header" style={{ gridTemplateColumns: `160px repeat(${days.length}, minmax(26px, 1fr))` }}>
        ...
        {days.map((day) => { const { jd } = isoToJalaliParts(day); return <span key={day} className={day === today ? "gantt-day-today" : ""}>{jd.toLocaleString("fa-IR")}</span>; })}
      </div>
      <div className="gantt-body" style={{ gridTemplateColumns: `160px repeat(${days.length}, minmax(26px, 1fr))` }}>
        ...
        {rows.map((row) => {
          if (row.kind === "group") return <div className="gantt-row gantt-row-group" key={row.key}><button type="button" className="gantt-row-label" disabled={!row.campaign} onClick={() => row.campaign && navigate("/campaigns")}>{row.campaign?.title ?? "بدون کمپین"}</button><span className="gantt-row-track" style={{ gridColumn: `2 / ${days.length + 2}` }} /></div>;
          if (row.kind === "campaign-bar") { const style = barStyle(days, row.campaign.startDate ?? "", row.campaign.endDate ?? ""); return <div className="gantt-row" key={row.key}><span className="gantt-row-label gantt-row-label-sub">بازه کمپین</span>{style && <button type="button" className="gantt-bar gantt-bar-campaign" style={style} onClick={() => navigate("/campaigns")} title={`${formatJalaliDate(row.campaign.startDate ?? "")} تا ${formatJalaliDate(row.campaign.endDate ?? "")}`} />}</div>; }
          const { content } = row;
