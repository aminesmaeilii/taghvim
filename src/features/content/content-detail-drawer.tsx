...
import type { Content, WorkspaceData } from "../../types/domain";
...
export function ContentDetailDrawer({ content, workspace, onClose }: { content: Content | null; workspace: WorkspaceData; onClose: () => void }) {
  ...
  const duplicate = async () => { await contentRepository.duplicateContent(content.id); invalidate(); pushToast({ title: "یک نسخه پیش نویس از محتوا ساخته شد." }); };
  const archive = async () => { await contentRepository.archiveContent(content.id); invalidate(); onClose(); pushToast({ title: "محتوا بایگانی شد." }); };
  const remove = async () => { if (window.confirm("این محتوا برای همیشه حذف شود؟")) { await contentRepository.deleteContent(content.id); invalidate(); onClose(); pushToast({ title: "محتوا حذف شد." }); } };
  const status = STATUS_META[content.status];
  return <Drawer open={Boolean(content)} title="جزئیات محتوا" onClose={onClose}>
    <div className="detail-drawer-content">
      <div className="detail-title"><StatusBadge status={content.status} label={status.label} color={status.color} /><h3>{content.title}</h3>{content.shortDescription && <p>{content.shortDescription}</p>}</div>
